...
* Filters `moz-fx-data-shared-prod.telemetry.crashes` to only those enrolled in the experiment. Performs subsequent aggregations at the client and `submission date` for specific crash types`.
   - Output Table: `r tbl.crashes`
   
The [second step analyzes](https://github.com/mozilla/fission-monitoring-nightly/blob/main/analysis_etl.Rmd) these tables for specific probes defined [here](https://github.com/mozilla/fission-monitoring-nightly/blob/main/params.R#L24).
