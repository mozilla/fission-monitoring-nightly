...
print(glue('Import - Crash table: {tbl.crash.raw}'))
tbl.crashes <- Sys.getenv('BQ_TMP_CRASH_TABLE')
print(glue('Import - Crash temporary aggregation table: {tbl.crashes}'))
...
min_build_date <- bq_project_query(project_id, build_min_build_id_query(tbl.main, num_build_dates)) %>%
  bq_table_download() %>%
  pull(max_build_date) %>%
  format('%Y-%m-%d')

bq_project_query(project_id, 
                 build_crash_table(tbl.main = tbl.main, tbl.crash.raw = tbl.crash.raw, 
                                   min_build_id = min_build_id, min_build_date = min_build_date), 
