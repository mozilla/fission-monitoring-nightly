...
```{r crash_import}
crashes <-  bq_project_query(project_id, build_crash_query(slug, main_tbl, max_build_id))
...
for (probe in names(probes.crashes)){
  ...
  results.crashes[[probe_per_hour]] <- scalar.df[get(probe_per_hour)<quantile(get(probe_per_hour), perc.high, na.rm = TRUE),
                                  summarize.scalar(.SD[,list(id,branch,x=1000*get(probe_per_hour))], "x", bs_replicates, stat=mean.narm),
